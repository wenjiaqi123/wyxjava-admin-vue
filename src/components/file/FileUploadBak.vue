<template>
  <div class="global-uploader">
    <!--上传-->
    <!--
    https://github.com/simple-uploader/Uploader/blob/develop/README_zh-CN.md#事件
      参数：
          options：      具体字段参考 data中的注释
          autoStart:     默认 true 是否选择文件后自动开始上传
      事件：
          @upload-start()       开始上传
          @file-added()         添加了一个文件，一般用做文件校验，如果给 file或者fileList 增加 ignored属性为true的话就会被过滤掉
          @file-success()       文件上传成功回调
          @file-progress()      文件进度的回调 ，上传过程会不断触发 progress 上传进度的回调
          @file-error()         文件上传失败回调
          @file-removed()       移除一个文件
          @files-submitted()    所选择的文件添加到上传队列后触发
     -->
    <uploader
      class="uploader-box"
      ref="uploader"
  ...
  export default {
    ...
    methods: {
      ...
      onFileAdded: function (file, fileList) {
        ...
        let data = {}
        this.axios.post(`/file/bFile/1111`, data)
          .then((resp) => {
          })
      },
      /**
